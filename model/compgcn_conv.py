...
class CompGCNConv(MessagePassing):
	...
	#def __init__(self, in_channels, out_channels, num_rels, act=lambda x:x, params=None):
		...
		self.num_rels 		= num_rels      # Relations in whole graph
		self.act 		= act               # activation pass through the model
		self.device		= None
		self.opn = opn
		self.bias = bias
	...
	def forward(self, x, edge_index, edge_type, rel_embed): 
		...
		self.in_index, self.out_index = edge_index[:, :num_edges], edge_index[:, num_edges:] # original, inverse edges
		self.in_type,  self.out_type  = edge_type[:num_edges], 	 edge_type [num_edges:]

		self.loop_index  = torch.stack([torch.arange(num_ent), torch.arange(num_ent)]).to(self.device) # self loop
