...
def create_test_graph(rule_graph_for_test, max_rule_length):
    ...
    for key, value in rule_graph.items():
        #print(key, 'Len of test batch: ', len(value))
        test_batch_graph = {}
        #if cnt > 100:
        #    break
        #cnt += 1
        if len(value)>0:
            for k, v in value.items():
                #print(k, 'Len of test sample: ', len(v))
                source_triplet = k.split('\t')[:-1]
                source_triplet = [int(element) for element in source_triplet]
                #print('Sample Source', source_triplet)
                label = k.split('\t')[-1]
                ...
                #print('Label', label)
                # Extract nodes, edges, and target nodes for this training graph
                #print('Len of triplets', len(triplets))
                if len(triplets)>0:
