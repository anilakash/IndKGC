...
def enc(rule_file, data_dir, remove_hops):
    ...
    rules_raw = pd.read_csv(rule_file, sep='\t', header=None)
    rules_raw = rules_raw.sample(frac=1).sort_values([2], ascending=False) # The rules are sorted
    rules = rules_raw[3].tolist()
    confidence = rules_raw[2].tolist()

    encoded_rules = []
    for i in range(len(rules)):
        line = rules[i]  # The rule head + body
        conf = confidence[i] # The rule's confidence
        ...
        if len(body)>remove_hops:
            ...
            encoded_rules.append((head_rel, body_rel_seq, conf)) # A tuple of head's relation and body and conf
            encoded_rules.append((head_rel_rev, body_rel_seq_rev, conf)) # A tuple of reversed head's relation, body, and conf
        #else:
        #    print('body', body)
